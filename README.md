...
│   ├── tests/
│   └── addons/
│       └── opencv_aruco/          # the addon: runtime node, export guard, bin/ with the GDExtension
│           └── bin/
...
The marker tracking itself ships as the self-contained addon
[`project/addons/opencv_aruco/`](./project/addons/opencv_aruco/README.md): a drop-in
replacement for Godot's built-in OpenXR marker tracking (markers surface as
`OpenXRMarkerTracker`s on the `XRServer`, consumed with `XRAnchor3D` — see the addon README
for the tracker contract). Copy that folder into another project to use it there — or grab
the prebuilt addon zip: the [`Build addon zip`](./.github/workflows/build_addon.yml) GitHub
Actions workflow (manual run, or any `v*` tag, which also creates a release) builds
debug+release binaries for Windows x86_64, Linux x86_64, macOS arm64 and Android arm64 and
packs them with the addon into one zip that extracts straight into a Godot project root.
...
| `project/addons/opencv_aruco/bin/opencv_aruco.gdextension` | Tells Godot which native library to load for each platform. |
...
4. Update the loaded library paths in [`project/addons/opencv_aruco/bin/opencv_aruco.gdextension`](./project/addons/opencv_aruco/bin/opencv_aruco.gdextension).
...
7. Optionally rename the `.gdextension` file itself, for example:
   ...
   ```text
   project/addons/opencv_aruco/bin/opencv_aruco.gdextension
   ```

   to:

   ```text
   project/addons/opencv_aruco/bin/your_extension.gdextension
   ```
...
```text
project/addons/opencv_aruco/bin/android/libopencv_aruco.android.template_debug.arm64.so
```
...
```bash
file project/addons/opencv_aruco/bin/android/libopencv_aruco.android.template_debug.arm64.so
```
...
```bash
nm -D project/addons/opencv_aruco/bin/android/libopencv_aruco.android.template_debug.arm64.so | grep opencv_aruco_library_init
```
...
```text
WSL:     ~/<repo>/project/addons/opencv_aruco/bin/android/libopencv_aruco.android.template_debug.arm64.so
Windows: <repo>\project\addons\opencv_aruco\bin\android\libopencv_aruco.android.template_debug.arm64.so
```
