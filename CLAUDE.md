...
- **`src/aruco_nano.h` may be included from exactly ONE translation unit** (currently `OpenCVProcessor.cpp`). The vendored ArucoNano header defines three non-inline statics, so a second `#include` fails the link on duplicate symbols. That is why `OpenCVProcessor.h` only forward-declares `namespace aruco_nano { class ArucoDetector; }`, why `detector` is a `unique_ptr` instead of a by-value member, and why the destructor must stay in the `.cpp` where the type is complete. Also note the detector must be constructed with the **vector** dictionary overload — the single-dictionary one appends to ArucoNano's default `{ARUCO_MIP_36h12}` instead of replacing it, silently searching two dictionaries.
