...
//ArucoNano is forward-declared, deliberately NOT included: aruco_nano.h defines three non-inline
//statics, so a second translation unit including it fails the link on duplicate symbols. The
//header is included in OpenCVProcessor.cpp only -- which is why `detector` is held by pointer
//rather than by value, and why ~OpenCVProcessor() must stay defined in the .cpp, where the type
//is complete.
